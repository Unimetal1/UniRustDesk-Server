...
fn make_libsodium(target: &str, source_dir: &Path, install_dir: &Path) -> PathBuf {
    if target.contains("windows") {
        let arch = if target.starts_with("i686") { "Win32" } else { "x64" };
        let profile = if env::var("PROFILE").map_or(false, |value| value == "release") {
            "Release"
        } else {
            "Debug"
        };
        return PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap())
            .join(format!("msvc/{arch}/{profile}/v142/"));
    }

    make_libsodium_from_source(target, source_dir, install_dir)
}

fn make_libsodium_from_source(target: &str, source_dir: &Path, install_dir: &Path) -> PathBuf {
    ...
    // Run `./configure`
    let to_msys_path = |path: &Path| {
        let path = path.to_string_lossy().replace('\\', "/");
        if cfg!(windows) && path.as_bytes().get(1) == Some(&b':') {
            format!("/{}/{}", path[..1].to_ascii_lowercase(), path[3..].to_string())
        } else {
            path
        }
    };
    let prefix = to_msys_path(install_dir);
    let prefix_arg = format!("--prefix={prefix}");
    let libdir_arg = format!("--libdir={prefix}/lib");
    let configure_script = source_dir.join("configure");
    let mut configure_cmd = if cfg!(windows) {
        let mut command = Command::new("sh");
        command.arg(to_msys_path(&configure_script));
        command
    } else {
        Command::new(configure_script)
    };
    if cfg!(windows) {
        configure_cmd.env("SHELL", "sh");
        if let Ok(ar) = env::var("AR_x86_64_unknown_linux_gnu").or_else(|_| env::var("AR")) {
            configure_cmd.env("AR", ar.replace('\\', "/"));
        }
        if let Ok(ranlib) =
            env::var("RANLIB_x86_64_unknown_linux_gnu").or_else(|_| env::var("RANLIB"))
        {
            configure_cmd.env("RANLIB", ranlib.replace('\\', "/"));
        }
    }

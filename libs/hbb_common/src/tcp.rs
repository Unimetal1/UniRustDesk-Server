...
use sodiumoxide::crypto::{box_, secretbox::{self, Key, Nonce}};
use std::{
...
#[derive(Clone)]
pub struct Encrypt(Vec<CipherLayer>);
    ...
    pub async fn next(&mut self) -> Option<Result<BytesMut, Error>> {
        ...
        if let Some(key) = self.2.as_mut() {
            if let Some(Ok(bytes)) = res.as_mut() {
                if let Err(error) = key.dec(bytes) {
...
impl Encrypt {
    ...
    pub fn dec(&mut self, bytes: &mut BytesMut) -> Result<(), Error> {
        ...
        // Transport encryption is added first and is therefore the outermost
        // layer on the wire. Remove layers in insertion order.
        for layer in &mut self.0 {
            layer.receive_sequence += 1;
            let nonce = FramedStream::get_nonce(layer.receive_sequence);
            decrypted = secretbox::open(&decrypted, &nonce, &layer.key)
                .map_err(|_| Error::new(ErrorKind::Other, "decryption error"))?;
        }
        bytes.clear();
        bytes.put_slice(&decrypted);
        Ok(())
    }

    pub fn enc(&mut self, data: &[u8]) -> Vec<u8> {
        let mut encrypted = data.to_vec();
        // Apply the newest (session) key first, then wrap it in the older
        // transport layer expected by the authenticated relay.
        for layer in self.0.iter_mut().rev() {
            layer.send_sequence += 1;
            let nonce = FramedStream::get_nonce(layer.send_sequence);
            encrypted = secretbox::seal(&encrypted, &nonce, &layer.key);
        }
        encrypted
    }
